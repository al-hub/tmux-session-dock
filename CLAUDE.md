...
`v0.3.69`; when it stops being true, fix it here in the same change.
...
| `v0.3.69` | `ensure_sidebar_for_session` skips subpane provisioning if another window holds the active lease, preventing background session creation from stealing subpanes across headless and client-attached environments. |
