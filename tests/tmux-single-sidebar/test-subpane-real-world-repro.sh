...
echo "Panes in sess2 after migration: $s2_count subpane(s) (Expected 2)"
...
echo "Panes in sess1 after roundtrip: $s1_count_back subpane(s) (Expected 2), Launcher height=$launcher_h_back (Expected >= 20)"

if [ "$s2_count" -ne 2 ] || [ "$s1_count_back" -ne 2 ] || [ "$launcher_h_back" -lt 15 ]; then
    echo "🚨 [DETECTED BUG 2]: Session roundtrip caused subpane loss, ghost duplication, or launcher collapse!"
    echo "   sess2 count=$s2_count, sess1 back count=$s1_count_back, launcher height=$launcher_h_back"
else
