...
There is no coordinator daemon. Every managed window runs one presenter (a bash TUI in a 34-column pane); they agree by reading and writing the same tmux server state - hidden global environment variables for transient bookkeeping, tmux options for durable identity, and filesystem locks where mutual exclusion is required - `mkdir`+pid for the subpane slots, and for the AI Activity Observer a pid file hard-linked into place, so that the claim exists complete or not at all and a holder that is slow to be scheduled is never mistaken for a dead one. The only extra processes on a server are the one AI Activity Observer and the Subpane Hub keeper.
...
5. **24-Phase Waveform Gradient**: The wave is configured by two options the presenter re-reads once a second, so a change from the `Prefix + S` popup lands on the next frame with no restart: `@session-dock-gradient` (`on`/`off`) and `@session-dock-gradient-speed`, one cycle in milliseconds, clamped to 400-4000. Both clocks - the microsecond frame clock the phase is computed against and the presenter's read timeout - derive from that single number in the pure `sidebar_domain_animation_timing`; setting one without the other would make the loop wake at a rate the frame clock does not match. Re-reading costs no round trip: the two options ride in the format string of the once-a-second prefetch the presenter already issues. Turning the gradient off also drops the read timeout back to the idle interval, so "off" stops the frame wakeups rather than only skipping the drawing. A session that has been silent for `@session-dock-awaiting-after` (30 s by default, counted from its last visible output and floored at the busy window so the number is never rounded up behind the user's back) and that no client has visited since is **Awaiting**: it gets a `●` in the second mark column and is counted in the header, because a list that scrolls cannot otherwise show that something below the fold wants attention. That column also carries `*` for the session a client is on, and the two can never collide - being on a session is exactly what ends Awaiting. Only the shared observer publishes the verdict (a presenter observing locally passes a zero threshold and never produces it, so two windows cannot contradict each other), and the first sighting of a session can never produce it, or every reopened dock would raise a false alarm ten seconds later. A clock never clears it: the blink may be limited to a budget, the state waits for the user. The dock draws the blink itself rather than handing it to the terminal as SGR 5, because too many terminals (Windows Terminal among them) accept that escape and render nothing, which turns the mark into a silent dot; it costs no extra wakeup, since the presenter loop is already awake several times a second and one cell is repainted with a bare `printf`. `@session-dock-awaiting-blink` takes `always` (the default - the mark blinks until the session is visited), `off` (a steady mark from the start) or a millisecond budget, after which the mark settles while the state itself remains. One shared AI Activity Observer per tmux server samples every AI pane once per second and publishes a state file (per-session state plus a topology hash and the attached-client list); presenters read it with builtins and collect only when that summary changed, so an idle presenter issues no tmux commands. Presenters write a heartbeat before every read; the observer's watchdog logs a presenter whose heartbeat stalls while its process is alive (`TMUX_SESSION_SIDEBAR_WATCHDOG=log`, default) and can send Escape to unblock it (`recover`), never while a prompt is open. Only the presenter with an attached client animates the wave, at 24 FPS (one cycle per second).
...
See also [ARCHITECTURE-EVOLUTION.md](ARCHITECTURE-EVOLUTION.md) for before/after diagrams of the eight points where the shape of the codebase changed, and [PERFORMANCE-ROADMAP.md](PERFORMANCE-ROADMAP.md) for the native core and zero-fork optimization roadmap.
...
| `v0.3.54` | Awaiting: the mark, the header count, and the rule that only the shared observer publishes the verdict (invariant 5) |
| `v0.3.56` | The awaiting header and mark broke every switch; the readiness oracle now matches what the renderer may draw, and Mark After is floored at the busy window |
| `v0.3.57` | Per-client questions go to `list-clients` (`sidebar_client_field`); a name wider than the row's name cell is recognised from its drawn prefix |
| `v0.3.61` | The AI Activity Observer claims its lock by hard-linking a pid file, and leaves if it is no longer the holder |
